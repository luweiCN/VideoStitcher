...
test('发布请求允许指定高于 master 的目标版本并交给 GitHub Actions', async () => {
  ...
  const management = createManagement(async (input, init) => {
    ...
    if (url.includes('/git/ref/tags/v3.5.0')) return new Response(null, { status: 404 });
  ...
  const operation = await management.publish('3.5.0', '修复批量处理问题');
  assert.equal(operation.kind, 'publish');
  assert.equal(operation.version, '3.5.0');
  ...
  assert.equal(body.inputs.version, '3.5.0');
...
test('目标版本不能低于 master 当前版本', async () => {
  ...
  await assert.rejects(
    management.publish('2.9.5', ''),
    /不能低于代码当前版本 2.9.6/,
  );
});

test('管理后台发布会触发独立部署 Workflow', async () => {
  let dispatchBody: { ref: string; inputs: Record<string, string> } | undefined;
  const management = createManagement(async (input, init) => {
    const url = String(input);
    if (url.includes('/actions/workflows/deploy-license-server.yml/runs?')) {
      return Response.json({ workflow_runs: [] });
    }
    if (url.endsWith('/actions/workflows/deploy-license-server.yml/dispatches')) {
      dispatchBody = JSON.parse(String(init?.body)) as { ref: string; inputs: Record<string, string> };
      return new Response(null, { status: 204 });
    }
    throw new Error(`未处理的测试请求：${url}`);
  });

  const operation = await management.deployAdmin();

  assert.equal(operation.kind, 'deploy-admin');
  assert.equal(operation.status, 'waiting');
  assert.equal(dispatchBody?.ref, 'master');
  assert.equal(dispatchBody?.inputs.deployment_request_id, operation.requestId);
});

test('版本面板展示正在运行的管理后台部署任务', async () => {
  const management = createManagement(async (input) => {
    const url = String(input);
    if (url.includes('/contents/package.json')) {
      return new Response(JSON.stringify({ version: '2.9.6' }));
    ...
    if (url.includes('/actions/workflows/deploy-license-server.yml/runs?')) {
      return Response.json({
        workflow_runs: [{
          id: 30001,
          display_title: '部署授权服务 · 42ef6fc4-6ac7-4ef6-89bb-1c433a89d87a',
          head_sha: 'd5aa2f9de924c87bfae44428f53743da12109934',
          status: 'in_progress',
          conclusion: null,
          html_url: 'https://github.example/runs/30001',
          created_at: '2026-07-22T02:08:34Z',
  ...
  assert.equal(dashboard.operations[0]?.kind, 'deploy-admin');
  assert.equal(dashboard.operations[0]?.status, 'in_progress');
  assert.equal(dashboard.operations[0]?.requestId, '42ef6fc4-6ac7-4ef6-89bb-1c433a89d87a');
});
...
test('TOS 目录尚未建立时也会通过私有标签阻止重复发布旧版本', async () => {
  ...
  await assert.rejects(management.publish('2.9.4', ''), /版本 2.9.4 已经发布/);
});
...
function createManagement(
  ...
  return new ReleaseManagementService({
    ...
    deployWorkflow: 'deploy-license-server.yml',
