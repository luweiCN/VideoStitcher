...
test('客户端首次连接自动创建一台授权设备的七天试用', async () => {
  const fixture = createFixture();
  const session = await fixture.service.connectDevice(deviceInput, '127.0.0.1');
  const license = session.license as { accessSource: string; expiresAt?: string };

  assert.equal(session.authorized, true);
  assert.equal(license.accessSource, 'trial');
  assert.equal(license.expiresAt, '2026-07-24T08:00:00.000Z');
...
test('更换绑定设备后，旧设备不能重新领取试用', async () => {
  const fixture = createFixture();
  const monthlyPlan = fixture.storage.database.plans.find((plan) => plan.code === 'monthly');
  assert.ok(monthlyPlan);
  const connected = await fixture.service.connectDevice(deviceInput, '127.0.0.1');
...
test('月度套餐结束后自动切换到排队中的年度套餐', async () => {
  ...
  assert.equal(queued.startsAt, '2026-08-24T08:00:00.000Z');
  assert.equal(queued.expiresAt, '2027-08-24T08:00:00.000Z');

  fixture.setTime('2026-08-24T08:00:00.000Z');
  const overview = await fixture.service.listAdminOverview();
...
test('试用期间重装不会创建第二条试用，获得设备套餐后不允许静默换凭据', async () => {
  const fixture = createFixture();
  const first = await fixture.service.connectDevice(deviceInput, '127.0.0.1');
  fixture.setTime('2026-07-20T08:00:00.000Z');
  const reinstalledInput = { ...deviceInput, installationId: 'new-trial-installation-id-00000001' };
...
test('过期试用设备可用一次性套餐码自助开通，重复请求不会重复发包', async () => {
  const fixture = createFixture();
  const monthlyPlan = fixture.storage.database.plans.find((plan) => plan.code === 'monthly');
  assert.ok(monthlyPlan);
  await fixture.service.connectDevice(deviceInput, '127.0.0.1');
  ...
  fixture.setTime('2026-07-25T08:00:00.000Z');
  const expiredSession = await fixture.service.connectDevice(deviceInput, '127.0.0.1');
  assert.equal(expiredSession.authorized, false);
  const before = await fixture.service.getPackageCenter(String(expiredSession.sessionToken));
  ...
  const redeemed = await fixture.service.redeemPackageCode(
    String(expiredSession.sessionToken),
    compactCode,
  );
