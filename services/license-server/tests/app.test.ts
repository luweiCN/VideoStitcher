...
test('后台不再支持按设备 ID 建档，设备只由客户端首次连接自动创建', async () => {
  ...
  assert.equal(connected.data.authorized, true);
...
test('管理员可以通过接口给用户追加套餐包', async () => {
  ...
  assert.equal(monthly.data.packageGrant.status, 'queued');
  assert.equal(monthly.data.packageGrant.startsAt, '2026-07-24T08:00:00.000Z');
  assert.equal(monthly.data.packageGrant.expiresAt, '2026-08-24T08:00:00.000Z');
  ...
  assert.equal(granted.data.packageGrant.startsAt, '2026-08-24T08:00:00.000Z');
  assert.equal(granted.data.packageGrant.expiresAt, '2027-08-24T08:00:00.000Z');
});
...
test('管理员生成套餐码后，客户端可查看套餐中心并完成一次性兑换', async () => {
  ...
  assert.equal(redeemed.data.packageCenter.packages[0]?.status, 'queued');
...
test('客户端首次连接可自动领取七天试用', async () => {
  const { application } = fixture();
  const trialRequest = () => createSignedDeviceRequest('/v1/devices/connect', {
      installationId: '018f5810-4f46-7c92-bb85-7c0695c39ea1',
      devicePublicKey,
      machineFingerprint,
      deviceName: '测试设备',
  ...
  const response = await application.handle(trialRequest());
  assert.equal(response.status, 201);
  const body = await response.json() as {
    data: { authorized: boolean; license: { accessSource: string; expiresAt: string } };
  };
  assert.equal(body.data.authorized, true);
  assert.equal(body.data.license.accessSource, 'trial');
  assert.equal(body.data.license.expiresAt, '2026-07-24T08:00:00.000Z');
});
