...
**创建文件**:
- ✅ `src/main/ipc/aside-handlers.ts` - IPC 处理器（392 行）
- ✅ `src/main/services/ProjectStorage.ts` - 项目存储（395 行）

**修改文件**:
- ✅ `src/main/langgraph/nodes/scriptNode.ts` - 添加进度回调和错误处理
- ✅ `src/main/langgraph/nodes/characterNode.ts` - 添加进度回调和错误处理
- ✅ `src/main/langgraph/nodes/storyboardNode.ts` - 添加进度回调和错误处理
- ✅ `src/main/langgraph/nodes/videoNode.ts` - 添加进度回调和错误处理
- ✅ `src/main/index.ts` - 注册 AI 处理器

**实现的 IPC 通道**:
1. `aside:generate-scripts` - 生成脚本（支持批量）
...
8. `aside:list-sessions` - 列出所有会话
9. `aside:delete-session` - 删除会话
