...
import { Eye, Edit3, RefreshCw, User, Film, Plus } from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {!hasContent ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-xs text-slate-600">成果将在此显示</p>
          </div>
        ) : activeTab === 'characters' ? (
          // 人物卡片
          <div>
            ...
            <button
              ...
              className="w-full py-3 border-2 border-dashed border-slate-700 hover:border-violet-600 rounded-xl text-slate-500 hover:text-violet-400 transition-colors"
            >
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 mb-4">
                ...
                >
                  {storyboard.scenes.map((scene) => (
                    <div
                      key={scene.id}
                      className="aspect-video bg-slate-900 rounded-lg flex items-center justify-center overflow-hidden"
                    >
                      {scene.imageUrl ? (
                        <img
                          src={scene.imageUrl}
                        ...
                        <div className="text-center p-2">
                          <p className="text-2xl font-bold text-slate-700">{scene.index}</p>
                          <p className="text-xs text-slate-600 mt-1">{scene.description}</p>
                        </div>
                      )}
                    </div>
                  ))}
