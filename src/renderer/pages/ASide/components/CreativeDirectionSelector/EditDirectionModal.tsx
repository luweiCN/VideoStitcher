...
interface EditDirectionModalProps {
  ...
  onClose: () => void;
  /** 保存回调 */
  onSave: (directionId: string, name: string, description?: string, iconName?: string) => void;
}

/**
 * 编辑创意方向弹窗组件
 */
export function EditDirectionModal({ direction, onClose, onSave }: EditDirectionModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave(direction.id, name.trim(), description.trim() || undefined);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="w-full max-w-md bg-neutral-900 border border-slate-800 rounded-xl shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
          <h2 className="text-xl font-semibold">编辑创意方向</h2>
          <button
            onClick={onClose}
