...
const HorizontalMode: React.FC<HorizontalModeProps> = ({ onBack }) => {
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold text-violet-400">横屏极速合成</h1>
          <button
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm transition-colors ${showPreview ? 'bg-violet-500/20 text-violet-400' : 'bg-slate-800 text-slate-400'}`}
          >
      ...
      {showHelp && (
        <div className="mb-6 p-4 bg-slate-900 border border-slate-800 rounded-xl">
          <h3 className="font-bold mb-2 text-violet-400">使用说明</h3>
          <ul className="text-sm text-slate-300 space-y-1">
            <li>• <strong>主视频 (B面)</strong>: 主要内容视频，将被缩放到 607x1080 居中显示</li>
      ...
      <div className={`flex gap-6 ${showPreview ? 'flex-row' : 'flex-col'}`}>
        ...
        <div className="flex-1 space-y-6 min-w-0">
          ...
          <button
            ...
            className="w-full py-4 bg-gradient-to-r from-violet-500 to-violet-600 hover:from-violet-600 hover:to-violet-700 disabled:from-slate-700 disabled:to-slate-700 disabled:cursor-not-allowed rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-2"
          >
            {isProcessing ? (
              <>
