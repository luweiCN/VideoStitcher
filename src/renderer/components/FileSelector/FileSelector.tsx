...
import {
  useRegisterSetFilesCallback,
  useRegisterGetFileCountCallback,
  useRegisterGetFilesCallback,
  FileSelectorGroupContext,
  type SelectorContext,
} from "./FileSelectorGroup";
...
const FileSelectorWithRef = forwardRef<FileSelectorRef, FileSelectorProps>(
  (props, ref) => {
    const {
      id,
      name,
  ...
  `;
        ...
        if (selectedPaths.length > 0) {
          ...
          if (result.filesToAdd.length > 0) {
            if (multiple) {
              ...
              onChange?.([
                ...files.map((f) => f.path),
                ...result.filesToAdd.map((f) => f.path),
              ]);
            } else {
    ...
    const handleFileInfoUpdate = useCallback((updatedInfo: FileItem) => {
      setFiles((prev) =>
        prev.map((f) => (f.path === updatedInfo.path ? updatedInfo : f)),
      );
    }, []);
    ...
    const renderFileIcon = (file: FileItem) => {
      ...
      if (file.type === "image")
        return <ImageIcon className={`w-4 h-4 ${iconColorClass}`} />;
          ...
          <div className={getListStyle().content}>
            {files.map((file, index) => (
              <Tooltip.Root key={`${file.path}-${index}`}>
                ...
                <FileTooltipContent
                  file={file}
                  onInfoUpdate={handleFileInfoUpdate}
                />
              </Tooltip.Root>
            ))}
          </div>
          ...
          <div className={headerStyle}>
            <div className="flex items-center gap-3">
              <div
                ...
                style={{
                  backgroundColor:
                    required && !hasFiles ? theme.primary : "#64748b",
                }}
              />
            ...
            <div className="flex items-center gap-2">
              <span
                className="text-xs font-mono px-2 py-1 rounded-full bg-black/50"
                style={{ color: theme.primaryLight }}
              >
                {files.length > 0 ? `已选 ${files.length} 个` : ""}
                {!multiple
                  ? "(单选)"
                  : maxCount
                    ? `(最多 ${maxCount} 个)`
          ...
          <div
            ...
            onMouseEnter={(e) => {
              if (!hasFiles) {
                // 使用 50% 透明度的主题色
                const primaryColor =
                  theme.primary.match(/#[0-9a-fA-F]{6}/)?.[0];
                if (primaryColor) {
...
FileSelectorWithRef.displayName = "FileSelector";

// 具名导出，用于 import { FileSelector } 语法
const FileSelectorExport = FileSelectorWithRef;
export { FileSelectorExport as FileSelector };

export default FileSelectorWithRef;
