...
export class TaskQueueManager {
  ...
  private async executeTask(
    ...
    switch (type) {
      ...
      case 'video_resize':
        await this.executeResizeTaskMethod(task, executor);
        break;
   ...
   * 执行智能改尺寸任务
   */
  private async executeResizeTaskMethod(
    task: Task,
    executor: TaskExecutor
  ): Promise<void> {
    this.addLog(task.id, 'info', '开始执行智能改尺寸任务');
    ...
    const result = await executeResizeTask(
      {
        id: task.id,
        files: taskFiles,
        config: task.config as any,
    ...
    if (result.success) {
      // 智能改尺寸可能有多个输出文件
      const outputs: TaskOutput[] = (result.outputs || []).map((path: string) => ({
        path,
        type: 'video' as const,
      }));
      this.handleTaskComplete(task.id, outputs);
   ...
   * 执行封面格式转换任务
   */
  private async executeCoverFormatTaskMethod(
    task: Task,
    executor: TaskExecutor
  ): Promise<void> {
    this.addLog(task.id, 'info', '开始执行封面格式转换任务');
    ...
    const result = await executeCoverFormatTask(
      {
        id: task.id,
        files: taskFiles,
        config: task.config as any,
